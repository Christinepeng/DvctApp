...
class MyCompaniesDataSource(
    private val fetchMyCompaniesUseCase: FetchMyCompaniesUseCase
) : PageKeyedDataSource<Long, CompanyResponse>() {
    ...
    override fun loadInitial(
        ...
        callback: PageKeyedDataSource.LoadInitialCallback<Long, CompanyResponse>
    ) {
        ...
        val disposableObserver = object : DisposableObserver<List<CompanyResponse>>() {

            override fun onNext(data: List<CompanyResponse>) {
        ...
        fetchMyCompaniesUseCase.execute(
            disposableObserver,
            FetchMyCompaniesUseCase.Params.forJobs(0, params.requestedLoadSize)
        )
    }

    override fun loadBefore(
        params: PageKeyedDataSource.LoadParams<Long>,
        callback: PageKeyedDataSource.LoadCallback<Long, CompanyResponse>
    ) {
    ...
    override fun loadAfter(
        ...
        fetchMyCompaniesUseCase.execute(
            disposableObserver,
            FetchMyCompaniesUseCase.Params.forJobs(
                params.key.toInt(),
    ...
    fun dispose() {
        fetchMyCompaniesUseCase.compositeDisposable.clear()
