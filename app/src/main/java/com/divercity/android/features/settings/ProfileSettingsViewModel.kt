...
constructor(
    ...
    val uploadProfilePictureResponse = MutableLiveData<Resource<User>>()
//    val navigateToSelectUserType = SingleLiveEvent<Boolean>()
    ...
    fun enableNotifications(enabled: Boolean) {
        ...
        if (!sessionRepository.getDeviceId().isNullOrEmpty() && !sessionRepository.getFCMToken().isNullOrEmpty()) {
            ...
            updateFCMTokenResponse.postValue(
                Resource.error(
                    context.resources.getString(R.string.error_notifications),
                    null
                )
            )
        }
    ...
    fun uploadPicture(pictureBase64: String) {
        uploadProfilePictureResponse.value = Resource.loading(null)
        ...
        uploadProfilePictureUseCase.execute(
            callback,
            UploadProfilePictureUseCase.Params.forUploadPic(pictureBase64)
        )
    }
