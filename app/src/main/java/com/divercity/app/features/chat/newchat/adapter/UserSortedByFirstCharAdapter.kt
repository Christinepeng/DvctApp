...
constructor() : PagedListAdapter<Any, RecyclerView.ViewHolder>(userDiffCallback) {
    ...
    private var listener: UserCharViewHolder.Listener? = null

    fun setRetryCallback(retryCallback: RetryCallback) {
        this.retryCallback = retryCallback
    }

    fun setListener(listener: UserCharViewHolder.Listener) {
        this.listener = listener
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            R.layout.item_user -> UserCharViewHolder.create(parent, listener)
